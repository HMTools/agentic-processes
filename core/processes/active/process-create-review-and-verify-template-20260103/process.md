...
# Process: Create review-and-verify Template

**Template**: create-process-template
**Status**: Running

## Current State
**Active Step**: PAUSED - Waiting for user to create missing process-steps
**Current Action**: Process paused - missing investigation steps need to be created
**Details**: 11 investigation steps are missing. User must create them before process can continue to Step 4.

## Description
Create a new process template named review-and-verify that Generic investigation and verification workflow for reviewing code, files, references, or any aspect of a codebase. This template will guide users through Review and verify file references are correct; Review and verify files are not project-specific; Any investigation task that requires systematic review, verification, and iterative fixes.

## Parameters
- `templateName`: review-and-verify
- `templatePurpose`: Generic investigation and verification workflow for reviewing code, files, references, or any aspect of a codebase
- `useCases`: Review and verify file references are correct; Review and verify files are not project-specific; Any investigation task that requires systematic review, verification, and iterative fixes
- `exampleParameters`: investigationScope, verificationCriteria, targetFiles (optional)

## Context
- `repository`: agentic-processes
- `templateDirectory`: core/processes/templates/
- `templateFile`: core/processes/templates/review-and-verify.md
...
- [x] Step 3: Validate required process-steps exist
...
- [ ] Step 4: Continuous Improvement & Learning
  ...
  - **Context**:
    - `processLogPath`: core/processes/active/process-create-review-and-verify-template-20260103/log.md
    - `processName`: Create review-and-verify Template
