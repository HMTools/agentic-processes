...
## Examples of Good Detailed Plans

### Example 1: API Layer Implementation

````markdown
# Detailed Plan: Implement API Layer

## Overview
This step creates the API layer for the partner offers feature, including controllers, request/response DTOs, and input validation. The API will follow REST conventions and use the existing service flow pattern.

## Parent Plan
Reference: plans/partner-offers-feature/plan.md - Step 1: Implement API Layer

## Q&A - Step-Specific Information Needed

- [ ] Q1: What is the exact endpoint path? (e.g., /api/v1/partners/{partnerId}/offers or /api/External/partners/{partnerId}/offers)
  - Context: Need to determine controller placement (v1/ vs External/ directory) and route definition
  - **Answer:** _[User provides answer here]_
  
- [ ] Q2: Should the GET endpoint support pagination? If yes, what are the default page size and max page size?
  - Context: Determines if we need to add PaginationRequest parameters and PagedResponse<T> wrapper
  - **Answer:** _[User provides answer here]_

- [ ] Q3: What are the required and optional fields for CreateOfferRequest?
  - Context: Need exact field names, types, and validation rules (Required, Range, MaxLength, etc.)
  - **Answer:** _[User provides answer here]_

## Low Level Design

### Component Overview
Create a new OffersController in the appropriate API version folder with CRUD endpoints for partner offers.

### Architecture Diagram
```mermaid
classDiagram
    class OffersController {
        -IOfferService _service
        -ILogger~OffersController~ _logger
        +Task~ActionResult~ApiResponse~OfferResponse~~~ GetById(string partnerId, string offerId)
        +Task~ActionResult~ApiResponse~List~OfferResponse~~~~ GetByPartner(string partnerId)
        +Task~ActionResult~ApiResponse~OfferResponse~~~ Create(string partnerId, CreateOfferRequest request)
        +Task~ActionResult~ApiResponse~OfferResponse~~~ Update(string partnerId, string offerId, UpdateOfferRequest request)
    }
    ...
    OffersController --> CreateOfferRequest
    OffersController --> OfferResponse
```

### Files to Create/Modify
- `WebApi/Controllers/v1/OffersController.cs` - Main API controller with CRUD endpoints
- `Contracts/Requests/CreateOfferRequest.cs` - DTO for creating offers
- `Contracts/Requests/UpdateOfferRequest.cs` - DTO for updating offers
- `Contracts/Responses/OfferResponse.cs` - DTO for offer responses
- `WebApi/Mapping/OfferMappingExtensions.cs` - Extension methods for Request→Arguments and Results→Response mapping

### Key Classes/Interfaces

**OffersController**
- Responsibility: Handle HTTP requests for partner offers
- Base: ControllerBase with [ApiController] and [AutoConstruct]
- Dependencies: IOfferService, ILogger<OffersController>
- Routes: [Route("api/v1/partners/{partnerId}/offers")]

**CreateOfferRequest**
- Responsibility: Define contract for creating new offers
- Properties: OfferName (required, max 200), Amount (required, > 0), ExpirationDate (required, future date)
- Validation: Data annotations

**OfferResponse**
- Responsibility: Define contract for offer data in responses
- Properties: OfferId, OfferName, Amount, CreatedAt, UpdatedAt, Status

### Data Flow
1. HTTP Request → OffersController endpoint
2. Request DTO validated by model validation
3. Request.ToArguments() maps to service arguments
4. Service method called: await _service.CreateOfferAsync(args)
5. Result.ToResponse() maps to response DTO
6. ApiResponse<OfferResponse>.Ok(response) wraps result
7. HTTP Response sent with 200 OK or appropriate error status

### Error Handling
- Model validation errors → 400 Bad Request with validation details
- Business rule violations → Service returns failure result → 400 Bad Request with error message
- Not found scenarios → 404 Not Found
- Unhandled exceptions → Global exception handler → 500 Internal Server Error

### Dependencies
- AutoCtor for dependency injection
- Existing ApiResponse<T> wrapper
- IOfferService (from Service layer - will be created in next step)
...
- [ ] 1.1. Create Request DTOs [Complexity: 3]
  - Details: Create CreateOfferRequest and UpdateOfferRequest in Contracts/Requests/
  - Output: Two request DTO classes with data annotations for validation

- [ ] 1.2. Create Response DTO [Complexity: 2]
  - Details: Create OfferResponse in Contracts/Responses/
  - Output: One response DTO class

- [ ] 1.3. Create Mapping Extensions [Complexity: 3]
  - Details: Create OfferMappingExtensions with ToArguments() and ToResponse() methods
  - Output: Static extension class in WebApi/Mapping/

- [ ] 1.4. Create OffersController [Complexity: 4]
  - Details: Implement controller with GET, POST, PUT endpoints
  - Output: Controller class in WebApi/Controllers/v1/

- [ ] 1.5. Test API with Swagger [Complexity: 2]
  - Details: Run application and verify endpoints appear in Swagger UI
  - Output: Verified endpoints are accessible and documented

## Testing Considerations
- Integration tests will verify full request/response flow (Step 5)
- Unit tests for mapping extensions if complex logic exists
- Controller logic is thin (just orchestration), so unit tests may not be needed

## Success Criteria
- [ ] All request and response DTOs created with proper validation
- [ ] Mapping extensions implemented for all conversions
- [ ] Controller created with all required endpoints
- [ ] Code follows AutoCtor pattern and service flow pattern
...
````markdown
# Detailed Plan: Implement Service Layer

## Overview
This step implements the business logic for the partner offers feature, including service interface, implementation, validators, and internal contracts (Arguments and Results).

## Parent Plan
Reference: plans/partner-offers-feature/plan.md - Step 2: Implement Service Layer

## Low Level Design

### Component Overview
Create offer management service with business logic for creating, retrieving, updating, and validating partner offers.
...
classDiagram
    class IOfferService {
        <<interface>>
        +Task~CreateOfferResult~ CreateOfferAsync(CreateOfferArguments args)
        +Task~GetOfferResult~ GetOfferByIdAsync(GetOfferArguments args)
        +Task~UpdateOfferResult~ UpdateOfferAsync(UpdateOfferArguments args)
    }
    
    class OfferService {
        -IOfferRepository _repository
        -IOfferValidator _validator
        -ILogger~OfferService~ _logger
        +Task~CreateOfferResult~ CreateOfferAsync(CreateOfferArguments args)
        +Task~GetOfferResult~ GetOfferByIdAsync(GetOfferArguments args)
        +Task~UpdateOfferResult~ UpdateOfferAsync(UpdateOfferArguments args)
    }
    ...
    class OfferValidator {
        +ValidationResult ValidateCreateOffer(CreateOfferArguments args)
        +ValidationResult ValidateUpdateOffer(UpdateOfferArguments args)
    }
    
    IOfferService <|.. OfferService
    OfferService --> OfferValidator
```

### Files to Create/Modify
- `Service/Managers/Interfaces/IOfferService.cs` - Service interface
- `Service/Managers/OfferService.cs` - Service implementation
- `Service/Validators/OfferValidator.cs` - Business rule validation
- `Contracts.Internal/Arguments/CreateOfferArguments.cs` - Input contract
- `Contracts.Internal/Results/CreateOfferResult.cs` - Output contract
- (Additional Arguments and Results for other operations)

### Key Classes/Interfaces

**IOfferService**
- Responsibility: Define contract for offer business operations
- Methods: CreateOfferAsync, GetOfferByIdAsync, UpdateOfferAsync
- Return Types: Strongly-typed Result objects

**OfferService**
- Responsibility: Implement business logic and orchestrate repository calls
- Dependencies: IOfferRepository, IOfferValidator, ILogger
- Pattern: [AutoConstruct] for DI

**OfferValidator**
- Responsibility: Validate business rules (e.g., expiration date is in future, amount within limits)
- Methods: ValidateCreateOffer, ValidateUpdateOffer
- Returns: ValidationResult with success/failure and error messages

### Data Flow
1. Controller calls IOfferService method with Arguments
2. Service validates arguments using OfferValidator
3. If validation fails, return failure Result
4. Service calls repository to perform data operation
5. Service maps repository entity to Result
6. Result returned to controller

### Error Handling
- Validation errors → Return failure Result with error messages
- Repository errors → Log and return failure Result
- Unexpected exceptions → Log and rethrow (global handler catches)

### Dependencies
- IOfferRepository (from Repository layer - will be created in next step)
- MongoDB.Driver (via repository)
- AutoCtor for dependency injection
...
- [ ] 2.1. Create Internal Contracts [Complexity: 4]
  - Details: Create Arguments (CreateOfferArguments, etc.) and Results (CreateOfferResult, etc.) in Contracts.Internal/
  - Output: Multiple argument and result classes with init properties

- [ ] 2.2. Create Service Interface [Complexity: 2]
  - Details: Create IOfferService in Service/Managers/Interfaces/
  - Output: Interface with method signatures

- [ ] 2.3. Create Validator [Complexity: 4]
  - Details: Create OfferValidator in Service/Validators/ with business rule validation
  - Output: Validator class with validation methods

- [ ] 2.4. Create Service Implementation [Complexity: 5]
  - Details: Create OfferService in Service/Managers/ implementing IOfferService
  - Output: Service class with business logic

- [ ] 2.5. Register Service in DI Container [Complexity: 1]
  - Details: Add service registration in WebApi/Registrars/ or Program.cs
  - Output: Service available for dependency injection

## Testing Considerations
- Unit tests will mock IOfferRepository and test business logic (Step 4)
- Focus on testing validation rules and error handling
- Test success paths and various failure scenarios

## Success Criteria
- [ ] All Arguments and Results created with proper immutability
- [ ] Service interface and implementation created
- [ ] Validator implements all business rules
